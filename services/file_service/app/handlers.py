...
from services.file_service.app.db.session import SessionLocal
...
def handle_files_upload_create(payload: dict, envelope: dict) -> dict:
    ...
    try:
        ...
        raise MessageError(422, "Файл не удалось прочитать как Excel workbook.") from exc
    ...
    if not parser.supports(file_metadata, {"sheet_names": sheet_names}):
        raise MessageError(422, "Unsupported source_type or workbook layout")
